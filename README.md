...
Sơ đồ dưới đây minh họa hành trình xử lý câu hỏi khớp chính xác 100% với tên các Node đã đăng ký trong `langgraph_pipeline.py`:
...
flowchart TD
    ...
    subgraph LangGraph ["Đồ thị điều phối đa Agent LangGraph"]
        Entry["risk_triage (Safety Guard Node)"]
        Entry -->|Cấp cứu / Nguy hiểm| Emerg["early_exit (Phản hồi Khẩn cấp 115)"]
        Entry -->|An toàn| Router["intent_router (Phân loại Ý định)"]
        
        Router -->|out_of_scope| OutScope["early_exit (Từ chối ngoài phạm vi)"]
        Router -->|faq / Chào hỏi| LLMNode["general_qa (Nhánh QA không RAG)"]
        Router -->|drug_query / Y khoa| RewriteNode["query_rewrite (T5 Query Rewriter ~0.05s)"]
        
        subgraph RAG ["Luồng Hybrid RAG Pipeline Chuyên sâu"]
            RewriteNode -->|Từ khóa tối ưu| Ret["hybrid_retrieval (Search Node: ChromaDB + BM25)"]
            Ret --> Grader["evidence_grading (Thẩm định bằng chứng)"]
            Grader -->|low_score| Web["trusted_search (Web Search Node: Vinmec, Tâm Anh)"]
            Web --> Grader
            Grader -->|high_score| Gen["answer_generation (Sinh lời khuyên kèm Trích dẫn)"]
        end
        
        LLMNode --> LocalLLM["Local LLM (Qwen3-4B-Thinking via llama-server)"]
    end
    
    Gen --> Validator["medical_validation (Kiểm định & Gắn cảnh báo)"]
    LLMNode --> Validator
